...
`pytest -v` 共 41 个用例全部通过,覆盖 LRU/协议/哈希环/服务器/复制/分片六大模块。

![pytest 41 passed](docs/pic/测试.png)

## 端到端演示

### `python3 demo.py`:一致性哈希分片 + 主从复制 + 手动故障切换

`demo.py` 一键跑通三分片客户端路由、master 写从 slave 同步、淘汰合成 `DEL` 传播、手动提升 slave 为新 master。

![demo.py 端到端演示](docs/pic/demo.png)

### tmux 多终端交互式演示:协议 + LRU 淘汰 + 主从复制 + READONLY

五个窗格同时呈现 master/slave 服务进程与三个 `nc` 会话,直接用裸协议证明 v1 五项要求全部跑通。

![多终端交互式演示](docs/pic/演示截图.png)
