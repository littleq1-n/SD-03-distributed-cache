...
- **WHEN** 对同一个 key 多次计算其归属节点(节点集合不变)
- **THEN** 每次都返回同一个物理节点

#### Scenario: 分布大致均匀

- **WHEN** 将大量随机 key 映射到含若干物理节点(每节点多个虚拟节点)的环上
...
- **WHEN** 向 N 个节点的环中新增 1 个节点
- **THEN** 仅约 1/(N+1) 的 key 归属发生改变,其余 key 归属不变

#### Scenario: 受影响 key 表现为 miss 而非数据丢失

- **WHEN** 某 key 因增删节点改变了归属,客户端访问新归属节点
