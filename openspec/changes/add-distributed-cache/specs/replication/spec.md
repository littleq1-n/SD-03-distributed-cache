...
- **WHEN** master 惰性或定期发现 key Y 已过期并删除
...
- **WHEN** master 的一次 `SET` 同时导致一次淘汰
- **THEN** `SET` 与对应 `DEL` 被打包进同一个 offset 的 effect batch 传播

#### Scenario: batch 原子回放

- **WHEN** slave 接收并回放一个含多条效果的 batch
- **THEN** 该 batch 的所有效果要么全部应用、要么全部不应用,slave 记录的已应用 offset 仅在整组成功后推进

#### Scenario: offset 单调递增且顺序一致

- **WHEN** master 连续应用多条写
- **THEN** 对应 effect batch 以与本地应用相同的顺序、携带递增 offset 进入复制流,slave 按该顺序回放
...
- **WHEN** slave(无论全新还是断线重连)连接 master
- **THEN** master 执行全量同步,发送快照与 `snapshot_offset`,slave 加载后其 keyspace 与 master 快照时刻一致,已应用 offset = `snapshot_offset`

#### Scenario: 快照期间的写不丢失

- **WHEN** master 在生成/发送快照期间收到新的写命令
...
- **WHEN** master 不可用,运维将某 slave 提升为 master
